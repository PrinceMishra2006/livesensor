...
class ModelEvaluation:
    ...
    def initiate_model_evaluation(self)->ModelEvaluationArtifact:
        ...
        try:
            ...
            # Load valid train and test data
            ...
            # Extract and convert target column
            y_true = df[TARGET_COLUMN]
            y_true.replace(TargetValueMapping().to_dict(),inplace=True)
            df.drop(TARGET_COLUMN,axis=1,inplace=True)


            # Load the nawly trained model
            train_model_file_path = self.model_trainer_artifact.trained_model_file_path
            train_model = load_object(file_path=train_model_file_path)
 
            # Initialize model resolver to fetch best model if exists
            model_resolver = ModelResolver()


            # If no previous model exists, accept current model directly
            if not model_resolver.is_model_exists():
                logging.info("No previous model found. Accepting current model by default.")
            ...
            # Load latest/best model from previous evaluation
            ...
            # Create and save evaluation artifact
            model_evaluation_artifact = ModelEvaluationArtifact(
            ...
            #save evaluation report to YAML
